...
def skeleton_render(
    ...
    if sound:
        ...
        if render:
            out = os.system(
                f"ffmpeg -loglevel error -stream_loop 0 -y -i {gifname} -i {audioname} -shortest -c:v libx264 -crf 26 -c:a libmp3lame -q:a 4 {outname}"
            )
    else:
        if render:
...
def skeleton_render_simple(
    ...
    # make a temporary directory to save the intermediate gif in
    temp_dir = TemporaryDirectory()
    gifname = os.path.join(temp_dir.name, f"temp.gif")
    anim.save(gifname)

    assert type(name) == str
    assert name != "", "Must provide an audio filename"
    audioname = f"{temp_dir.name}/tempsound.wav"
    sf.write(audioname, sound, 44100)
    
    outname = os.path.join(
        out, f"{os.path.splitext(os.path.basename(name))[0]}.mp4"
    )
    
    ## merge sound and motion
